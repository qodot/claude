---
allowed-tools: Write(*), Read(*), Bash(mkdir:*), WebFetch(*), MCPSearch(*), mcp__plugin_playwright_playwright__browser_navigate(*), mcp__plugin_playwright_playwright__browser_snapshot(*), mcp__plugin_playwright_playwright__browser_close(*)
...
**WebFetch 실패 시 Playwright MCP 사용**:

```
1) MCPSearch로 Playwright 도구 로드
   - query: "select:mcp__plugin_playwright_playwright__browser_navigate"
2) browser_navigate로 URL 탐색
3) 페이지 스냅샷에서 필요한 정보 확인
4) 작업 완료 후 browser_close로 브라우저 종료
```

**중요**: 반드시 최신 문서를 확인해야 합니다. 문서 확인 없이 진행하지 마세요.
...
NOTE: 위 예시에서 느낌표(!) 뒤에 보이지 않는 문자가 삽입되어 실행을 방지합니다.
실제 사용 시에는 느낌표와 백틱을 직접 붙여 사용하세요.
