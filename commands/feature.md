...
**자동 처리:**
- 프로젝트 이름: 현재 디렉토리에서 자동 추출
- 브랜치 이름: PR 제목에서 자동 생성 (kebab-case)

**예시:**
```bash
# 명령어 실행 (현재 디렉토리: spray/)
/feature

# 질문 1: PR 제목을 입력하세요
# → 캠페인 목록에서 크리에이터 이메일로 검색 기능 추가

# 질문 2: 기능 설명 (선택사항, Enter로 건너뛰기)
# → 이메일 주소로 크리에이터를 빠르게 찾을 수 있는 검색 기능

# ... 이후 3개 질문 계속

# 자동 생성:
# - 프로젝트: spray
# - 브랜치: search-creator-by-email
```
...
이 기능의 PR 제목을 한 줄로 작성하세요.
(예: 캠페인 목록에서 크리에이터 이메일로 검색 기능 추가)

브랜치 이름은 이 제목에서 자동 생성됩니다.

답변을 입력하세요:
```

사용자 답변을 받은 후 **즉시 브랜치 이름 생성**:

```python
# PR 제목을 kebab-case 브랜치 이름으로 변환
import re

pr_title = "<사용자 답변>"

# 1. 소문자 변환
branch_name = pr_title.lower()

# 2. 한글 및 특수문자 제거, 영문/숫자/공백/하이픈만 유지
...
- 이후 모든 작업은 현재 디렉토리 내에서 진행

### 3단계: 계획 저장 (plan.yml)

**1단계에서 수집한 답변을 plan.yml에 저장:**

`.claude/features/<브랜치-이름>/plan.yml` 파일 생성:

```yaml
feature_name: <브랜치 이름 (PR 제목에서 자동 생성)>
project_name: <프로젝트 이름 (현재 디렉토리에서 자동 추출)>
description: <질문2 답변>
created_at: <타임스탬프>
...
파일: .claude/features/<브랜치-이름>/plan.yml
...
3. **기능 브랜치 생성 (1단계에서 생성한 브랜치 이름 사용)**
   ```bash
   # 브랜치 이름은 이미 1단계에서 PR 제목으로부터 생성됨
   git checkout -b <브랜치-이름>
   git push -u origin <브랜치-이름>
   ```
   - 브랜치 이름: PR 제목에서 자동 생성된 kebab-case
   - 원격에 푸시하여 업스트림 설정
...
**`.claude/features/<브랜치-이름>/spec.md` 생성:**

인터랙티브 모드로 수집한 답변을 반영하여 상세한 명세 작성:
...
3. **초기 테스트 구조 작성**
   - 인터랙티브 모드의 질문5 답변 기반으로 테스트 케이스 스텁 생성
   - 최소한 하나의 기본 테스트 추가
   - 실행하여 실패 확인 (TDD)
...
```
TODO 2: 테스트 작성
→ 질문5 답변 기반으로 테스트 케이스 구현
→ 테스트 실행하여 실패 확인
→ /commit을 실행하여 자동 커밋
→ TODO 2 완료 표시

TODO 3: 핵심 기능 구현
→ 질문4의 "포함 사항" 기반으로 코드 작성
→ 테스트 실행하여 통과 확인
→ /commit을 실행하여 자동 커밋
→ TODO 3 완료 표시
...
```
✅ 프로젝트: spray
📁 경로: /Users/user/projects/backend/projects/spray
```
...
plan.yml 저장됨: .claude/features/<브랜치-이름>/plan.yml
...
- **항상 5개 질문으로 정보 수집** - argument 없이 실행됨
- **프로젝트 이름은 현재 디렉토리에서 자동 추출**
- **브랜치 이름은 PR 제목에서 자동 생성 (kebab-case)**
...
- 모든 파일 생성/수정은 현재 디렉토리 내에서만 수행

## 성공 기준

기능 개발 워크플로우가 성공적일 때:

1. **간소화된 질문**: 5개 질문만으로 핵심 정보 수집 (기존 7개에서 2개 축소)
2. **스마트 자동화**: 프로젝트 이름과 브랜치 이름을 자동으로 추출/생성
3. **준비 완료**: 모든 설정 작업 자동화되고 완료됨
4. **계획 명확성**: 명확하고 실행 가능한 명세 문서 생성됨
5. **테스트 준비**: 테스트 구조가 준비되고 실행 가능함
6. **진행 상황 가시성**: TODO 리스트가 작업을 적극적으로 추적함
7. **Git 자동화**: 각 TODO 완료 시 자동으로 커밋되어 논리적 단위로 분리됨
8. **PR 자동 생성**: 모든 구현 완료 후 자동으로 Draft PR이 생성됨
9. **완전 자동화**: 개발자는 코드 작성에만 집중하고, Git/PR 워크플로우는 자동 처리

궁극적인 목표는 모든 설정 마찰과 반복 작업을 제거하고, **불필요한 질문을 최소화**하며, 개발자가 전적으로 기능 구축에만 집중할 수 있도록 하는 것입니다.
