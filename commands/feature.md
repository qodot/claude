---
allowed-tools: Bash(git:*), Bash(cd:*), Read(*), Write(*), Edit(*), Grep(*), Glob(*), TodoWrite(*)
...
→ 테스트 실행하여 실패 확인
→ git commit -m "test: Add test cases for <기능>"
→ TODO 2 완료 표시

TODO 3: 핵심 기능 구현
→ 질문3의 "포함 사항" 기반으로 코드 작성
→ 테스트 실행하여 통과 확인
→ git commit -m "feat: Implement <기능>"
→ TODO 3 완료 표시

TODO 4: 에러 처리
→ 예외 처리 및 엣지 케이스 코드 추가
→ 관련 테스트 추가
→ git commit -m "feat: Add error handling for <기능>"
→ TODO 4 완료 표시

TODO 5: 문서 작성
→ README 업데이트
→ 인라인 주석 추가
→ git commit -m "docs: Update documentation for <기능>"
→ TODO 5 완료 표시
...
→ 코드 정리 및 포맷팅
→ 불필요한 주석 제거
→ git commit -m "chore: Prepare for code review"
→ TODO 7 완료 표시
```
...
이 명령어를 실행하면 자동으로 PR이 생성됩니다.
인터랙티브 모드로 시작했기 때문에 PR 제목과 설명이
자동으로 채워집니다!
...
다음 단계:
1. 구현 옵션 선택 (자동/단계별/직접)
2. 개발 진행
3. /pr --draft 실행하여 PR 생성

주의: PR 생성 시 <프로젝트-이름> 디렉토리에서 실행하세요!
```
...
5. **Git 깨끗함**: 커밋되지 않은 작업이 없는 적절한 브랜치 설정
6. **개발자 준비**: 개발자가 명확한 안내와 함께 즉시 코딩 시작 가능
7. **PR 준비**: `/pr --draft` 실행만으로 PR 생성 가능

궁극적인 목표는 모든 설정 마찰을 제거하고 개발자가 전적으로 기능 구축에만 집중할 수 있도록 하는 것입니다.
