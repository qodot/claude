...
- 작은 PR 유지
- **커밋 분리**: 각 커밋이 논리적 단위인지 확인
  - 하나의 커밋에 모든 변경이 들어가 있으면 안 됨
  - 서비스/API/테스트 등 논리적 단위로 분리 필요
- 명확한 설명 (기술 + 비즈니스 맥락)
- 테스트 포함
- API 변경 시 문서 동기화
...
PR 생성 전에 커밋이 잘 분리되었는지 확인:
```bash
git log main..HEAD --oneline
```

**경고할 상황**:
- 커밋이 1개뿐인데 여러 기능 변경이 포함된 경우
- 커밋 메시지가 너무 광범위한 경우 (예: "기능 구현" 등)
